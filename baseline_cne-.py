...
def main():
    ...
    for epoch in range(300):
        ...
        out_y1, out_yc0, out_y0, out_yc1, treat_prob = model3(botData_train.x, botData_train.edge_index, treat_idx_train, control_idx_train)
        ...
        loss = loss_y*par['ly'] + loss_judgetreat*par['ljt']
        loss.backward()
        optimizer.step()

        if epoch%10 == 0:
            model3.eval()
            out_y1, out_yc0, out_y0, out_yc1, treat_prob = model3(botData_test.x, botData_test.edge_index, treat_idx_test, control_idx_test)
