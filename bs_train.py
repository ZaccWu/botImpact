...
# model parameters
# ignite, gial, cne-, dd
parser.add_argument('--model', type=str, help='model test', default='dd')
# training parameters
...
def main():
    ...
    if args.model == 'ignite':
        model_g = IgniteGenerator(in_dim=1, h_dim=16, out_dim=1)
        model_d = IgniteDiscriminator(h_dim=16)
        ...
        model_g = GialGenerator(in_dim=1, h_dim=16, out_dim=1)
        model_d = GialDiscriminator(h_dim=16)
        optimizer_g = torch.optim.Adam(model_g.parameters(), lr=0.001)
        optimizer_d = torch.optim.Adam(model_d.parameters(), lr=0.001)
        treat_idx, control_idx = torch.nonzero(botData_f.y[:, 2] == -1).squeeze(), torch.nonzero(
            botData_f.y[:, 2] == 1).squeeze()
    elif args.model == 'cne-':
        model = CNE_Minus(in_dim=1, h_dim=16, out_dim=1)
        optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': 0.001}])
        treat_idx, control_idx = torch.nonzero(botData_f.y[:, 2] == -1).squeeze(), torch.nonzero(
            botData_f.y[:, 2] == 1).squeeze()
    elif args.model == 'dd':
        model = DD(in_dim=1, h_dim=16, out_dim=1)
        optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': 0.001}])
        treat_idx, control_idx = torch.nonzero(botData_f.y[:, 2] == -1).squeeze(), torch.nonzero(
            botData_f.y[:, 2] == 1).squeeze()
    ...
    for epoch in range(args.epoch):
        if args.model == 'ignite':
            ...
            target_judgetreat = torch.cat(
                [torch.ones(len(treat_prob[treat_idx])), torch.zeros(len(treat_prob[control_idx]))])
        ...
        if epoch%10 == 0:
            ...
            if epoch == args.save_epoch:
                Z_emb, treat_lb = z.detach().cpu().numpy(), botData_f.y[:, 2].detach().cpu().numpy()
                if args.save:
                    savedt_path = 'result/save_emb/'+args.type+'/'
                    if not os.path.isdir(savedt_path):
                        os.makedirs(savedt_path)
                    np.save(savedt_path+args.model+str(seed)+'.npy', np.concatenate([Z_emb,treat_lb], axis=-1)) # emb_H + treat (dim: z+1)
